...
**每个 socratic 问题必须配 2-4 个候选答案选项**（AskUserQuestion 自动追加 "Other" 让学生写自由回答），选项设计原则：
...
- 鼓励学生在认知不确定时主动选 "Other" 写自己的话——这是费曼输出的入口
- 学生选 "Other" 时 AI 按 `pedagogy/feynman-technique.md` 评估其自由回答；选预置选项时 AI 根据选择判断理解深度并解释为何对/错（轻量纠正，非 stuck escalate）
...
1. **降低响应摩擦**（lower response friction）：用户在终端一键选择，相比读长段落再敲键盘回复，参与门槛显著降低
2. **结构化学习痕迹**（structured learning trace）：每次选择被工具调用日志保留，对比"自由文本回复"更易做学习分析
3. **认知约束就是教学**（cognitive constraint as pedagogy）：被迫从 2-4 个选项中选一个，比"随便答"更激活辨别性思维（discriminative thinking）——这是测试效应（testing effect）的高效实现，与 `pedagogy/spaced-repetition.md` 中描述的 retrieval practice 完美契合
...
| **多 topic 选择**（无 `_active` 且 ≥1 个 slug 目录）| 列活跃 topic 让学生选 | 各 topic 名（最近更新者标"（推荐）"放首位）+ "新建主题"|
| **动作分叉确认**（"现在出 task / 再讲一轮"）| AI 给推荐 + 备选 | 推荐动作标"（推荐）" + 1-2 备选 |
| **task 验收**（学生交付后）| 确认交付状态 | "已交付（推荐）/ 还需修改 / 跳过此任务" |
| **stuck-detected hint 后**| 简化问询学生是否仍卡 | "我有思路了 / 还是不太明白" |
...
- **永远不替学生构造"完美自由回答"**——把 "Other" 留给他/她的真实表达
- 题目主体应包含必要上下文（学生不需翻回上文也能理解问题）
...
- **`## 当前位置`**：学生的学习坐标。形如 `bloom: <level> | 已覆盖: <concept1>, <concept2>, ...`。**"已覆盖"清单是概念追踪的权威源**（取代旧设计中由 journal 各 lecture 摘要承担的追踪职责），每次 `accept` 时 AI 追加新概念名（用 `, ` 分隔）。更新时机：仅 accept / assemble。
...
`/learn` 采用**里程碑式落盘**（milestone-based persistence）：lecture / socratic / task 三类"纯教学动作"不写盘，内容留在对话上下文里；只有当状态机有真实变更（计数器、`bloom_level`、`artifact_count`、`status`、目录移动）时才触发落盘。设计目的是降低主对话上下文中的写文件噪声，让用户的学习专注力不被频繁的 Write/Edit 工具调用打断。
...
| `task`（描述下发）| ❌ 过渡态 | 不写（任务描述在对话上下文中可被学生回顾）|
...
- 间隔复习（spaced review）的时间戳信号锚点从"哪天 lecture 讲的"转为"哪天 accept 验收的"——更准确（间隔复习关心的是"理解过的概念何时复现"）
...
| `/learn`（无参 + 无 `_active` 但存在多个 topic）| **AI 主动列出所有 topic 及其状态**，请用户选择继续哪个或开新主题 |
| `/learn`（无参 + `topics/` 为空）| 询问用户首个主题 |
| `/learn <topic-name>` | 切换到该 topic 或新建 |

注意：本 skill 单一入口，不接受 `--list` `--status` 等子命令。所有"列出""归档""索引"等管理动作由 AI 在调度过程中**自动判断并执行**，不依赖用户显式触发。
...
- 行为：AI 读取所有 `topics/<slug>/state.md`，向用户输出表格形式的 topic 清单（topic 名 / status / bloom_level / artifact_count / updated_at），询问继续哪个或新建
- 用户选定后写 `_active` 指针并按选择 topic 推进
...
lesson.md 是学生的**首要阅读面**（primary reading surface）——追求**认知流**（cognitive flow）而非全面性（completeness）。notes.md 负责工业细节的完整性，lesson.md 负责"读完就懂"。以下 8 条规则是强制契约：
...
- **禁止以历史债务、背景故事、或前版回顾开头**——读者需要先知道"我在学什么"再了解"它从哪来"

**Rule 2 — "怎么跑"物理锚点（mandatory，位于前 1/3）**：
- 给出精确运行命令 + 预期输出形状（2-3 行描述关键现象）
- 即使复杂 lesson 也需要——学生需要"我能跑起来看到东西"的物理安全感
