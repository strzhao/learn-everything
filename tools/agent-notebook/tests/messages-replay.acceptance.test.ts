// 红队验收测试 —— runs[0].snapshots 演化正确性（契约 6 + 验收场景 3）
//
// 覆盖：
...
// 全部来自同一 run-log.txt → runs.length === 1，断言 runs[0].snapshots。
...
type Run = { id: string; label: string; snapshots: MessagesSnapshot[] };
...
// fixtures/normal 只有一份 run-log.txt → runs.length === 1
function snapsOf(j: LessonResp): MessagesSnapshot[] {
  expect(j.runs.length).toBeGreaterThanOrEqual(1);
  return j.runs[0].snapshots;
}

describe("runs[0].snapshots 基础结构（契约 6）", () => {
  test("runs[0].snapshots 是数组且非空", async () => {
    const j = await fetchLesson();
    const snaps = snapsOf(j);
...
describe("snapshot 数量与 log block 对应关系（场景 F）", () => {
  test("snapshot 数量 ≥ 含 round 的 log block 数量（≥2）", async () => {
    ...
    expect(snaps.length).toBeGreaterThanOrEqual(roundLogs.length);
    expect(snaps.length).toBeLessThanOrEqual(totalLogs);
  });
});
...
describe("messages 数组长度严格递增（场景 F + 场景 3 演化）", () => {
  test("snapshot[i].messages.length < snapshot[i+1].messages.length", async () => {
    const j = await fetchLesson();
    const snaps = snapsOf(j);
    const lengths = snaps.map((s) => s.messages.length);
  ...
  test("第一个 snapshot 的 messages 长度 ≥ 1（至少含初始 user prompt）", async () => {
    const j = await fetchLesson();
    expect(snapsOf(j)[0].messages.length).toBeGreaterThanOrEqual(1);
  });
});
  ...
  test("后续 snapshot 的 addedIndices 都落在当前 messages 范围内", async () => {
    ...
    for (let i = 1; i < snaps.length; i++) {
      ...
      for (const idx of s.addedIndices) {
        ...
        expect(idx).toBeGreaterThanOrEqual(snaps[i - 1].messages.length);
      }
    }
  });
...
describe("messages 演化与 Otter 真实 run-log.txt 一致（fixtures/normal 引用真实数据）", () => {
  test("最终 snapshot 的 messages 数量 = 6（来自真实 FINAL MESSAGES）", async () => {
    const j = await fetchLesson();
    const snaps = snapsOf(j);
