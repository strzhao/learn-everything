# Task 04：v4 Coordinator + Subagent Fork

> 把 Lecture 04 抽象出的 4 条 multi-agent 洞察（agent-role 是物理维度 / subagent 上行路由 / context 从深度变广度 / 判决统一-执行多态）落到 284 行 v4 代码 + 4 份真实 run-log + 教学叙事 lesson.md，对照 claude-code 工业 `AgentTool`（spawn sub-agent）验证每一条。

> **命名修订（2026-05-30）**：本课原标题 "coordinator-swarm" 名实不符——工业 "swarm" = "team"（多进程协作系统：mailbox + 共享 task list + idle 状态机），见 lesson 13。v4 实际做的是**同进程 fork 出 sub-agent 跑独立 messages 数组、同步返回 summary**，对应工业 `AgentTool`（`subagent_type=general-purpose`），故重命名为 `04-subagent-fork`。代码里 `"swarm-worker"` role 字面、`swarmId`、`spawn_swarm` 等保留作 historical code identifier（5-12 lesson 共享这套 role 名）。
...
~/.bun/bin/bun run /Users/stringzhao/workspace/learn-everything/tools/agent-notebook/server.ts \
  /Users/stringzhao/workspace/learn-everything/topics/agent-harness-engineering/artifacts/04-subagent-fork/
```
