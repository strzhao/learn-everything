// Task 06 v6: mini hook engine —— hook 系统作为通用 cross-cutting 旁路广播
// v5 (compact sub-system) 不动 + 新增 §8 HookRegistry / §9 Handler 形态 + §5 dispatch 内 emit PreToolUse/PostToolUse + §7 maybeCompact 内 emit PreCompact/PostCompact
// 核心 distinction（Socratic 07 内化）: sub-system = in-process critical path 必须可靠 / hook = cross-cutting 旁路 可失败可超时
// 对照: claude-code/src/utils/hooks/{AsyncHookRegistry.ts (Map<id,PendingHook> + Promise.allSettled),execAgentHook/execHttpHook/execPromptHook.ts 三种 handler,ssrfGuard.ts IP block},entrypoints/sdk/coreTypes.ts:25-53 HOOK_EVENTS 27 项
...
// ---------- 11. 配置 + callModel + 启动入口（注册默认 hooks）----------
...
audit(`[BOOT] role=${role} mode=${mode} hooks=${hookSet}`);
