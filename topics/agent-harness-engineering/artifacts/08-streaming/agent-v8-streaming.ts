// Task 08 v8: mini streaming agent —— v7 全继承 + streaming dispatch (pipelining)
// v7 §1-13 字面不动 + §14 StreamingToolExecutor + §15 callModelStreaming/runStreamingRound/--stream 开关
// 对照: StreamingToolExecutor.ts:40-531 / grouping.ts:29-31 "yield order, not concat order" / query.ts:838-862
...
audit(`[BOOT] role=${role} mode=${mode} hooks=${hookSet} stream=${streamMode} OTEL_LOG_USER_PROMPTS=${process.env.OTEL_LOG_USER_PROMPTS ?? "(unset, redacting)"}`);
try {
