...
**还有第 6 条 cache 经济债**（这一条 v10 task 不解决，留给未来 task）：v5 task 05 的 `microCompact` 也会炸 messages 字段的 cache prefix（client 改 `messages[].content` 字节 = byte-level prefix 哈希失效）。完整讨论见 [task 05 lesson §14 retrospective](../05-context-compactor/lesson.md) + [task 05 notes §7.5](../05-context-compactor/notes.md)——这两段是学完 v10 后回看 v5 才补的 retrospective disclaimer。工业 claude-code 用 `cache_edits` + `cache_reference` 协议（client `messages` 字面不变 / server 端按引用删 KV）才同时省 token + 保 cache，但这是 Anthropic 内部 feature（`CACHED_MICROCOMPACT`），公开 API 用户拿不到。

完整图景：cache 经济同权 = system 字段（v10 sectionCache + cache_control marker） + tools 字段（独立 marker） + messages 字段（cached microcompact + cache_edits）三个体系协同。v10 只解决了第一个。
