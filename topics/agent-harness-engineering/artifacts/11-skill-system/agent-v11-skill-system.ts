// Task 11 v11: Mini Skill System —— v10 全继承 + 新增 §23/§24/§25/§26 skill 子系统
// v10 §1-22 字面不动（仅 §3 +15 / §5 +12 / §10 入口 +10 共 3 处最小侵入）+ §21 assembleSystemPrompt 字面不动
// 新增四段：§23 loadSkillsFromDir + §24 SkillTool + getPromptForCommand + §25 buildSkillListingReminder + §26 executeShellInSkill
// 对照: claude-code src/skills/loadSkillsDir.ts:344-401 createSkillCommand.getPromptForCommand / :407-480 loadSkillsFromSkillsDir
// 对照: src/utils/promptShellExecution.ts:49-143 BLOCK_PATTERN+INLINE_PATTERN+Promise.all+permission check
// 对照: src/tools/SkillTool/SkillTool.ts:1101-1108 inline newMessages user-role text / :122-289 executeForkedSkill / utils/forkedAgent.ts:191-232 prepareForkedCommandContext
// 对照: src/utils/attachments.ts:2661-2751 getSkillListingAttachments / src/utils/messages.ts:3097-3099 wrapInSystemReminder
// 核心论断：(1) skill = 行为策略注入 (新 tool + user-role text 注入 + 临时 allowed-tools) 五系统字面不动 → 架构正交性第五次验证
//          (2) skill 正文走 user-role text 通道（工业 user-role text isMeta + newMessages / 教学版用 tool_result 当 user-role content）
//          (3) skill listing 走 attachment <system-reminder>（首轮 prepend / dynamic 不锁死在 cacheable system prompt 跟 v10 mcp_instructions DANGEROUS_uncached 同源动机）
//          (4) shell 加载期 vs 运行期正交（shell 永远在主 agent getPromptForCommand 内部跑 + 临时 alwaysAllow / fork 是后续把成品 task 传给 worker 的运行期隔离）
// v11 教学简化：MCP skill 跳 shell（loadSkillsDir.ts:371-374 同源 task 02 双层防御）+ ${CLAUDE_SESSION_ID} + powershell + fork 两层 alwaysAllow 第二次 propagate 不做
...
function getToolsForRole(role: Role): any[] {
  ...
  if (role === "coordinator") return [...BASE_TOOLS, ASK_USER_TOOL, SPAWN_SWARM_TOOL, SKILL_TOOL, ...mcpToolsExtra];
  return [...BASE_TOOLS, ASK_USER_TOOL, SKILL_TOOL, ...mcpToolsExtra];
}
