// Task 12 v12: Mini TodoWrite System —— v11 全继承 + 新增 §27/§28/§29 TodoWrite 子系统
// v11 §1-26 几乎字面不动；仅最小侵入 4 处把 agentId 沿 dispatch 链穿下去（per-agent 隔离的物理前提）：
//   §3 getToolsForRole +TODO_WRITE_TOOL / §5 dispatch+dispatchInner+execute 各 +agentId 形参 + execute 加 TodoWrite 分支
//   §10 runRounds +agentId 形参 + 每轮 callModel 前调 maybeInjectTodoReminder / StreamingToolExecutor +agentId / 3 个 run* 传 todoKey
// 新增三段：§27 TodoWriteTool（state map + schema 无 list-level refinement + continuous nudge + verification nudge + completed→clear）
//          §28 fixed-interval reminder（缺席探测器：turnsSinceWrite≥N 才注入 <system-reminder>）
//          §29 transcript-derived restore（倒扫 messages 找最后一个 TodoWrite tool_use 反序列化 input.todos —— 无文件持久化）
// 对照: claude-code src/tools/TodoWriteTool/TodoWriteTool.ts:65-114 call()（零 validation）+ mapToolResultToToolResultBlockParam（continuous + verificationNudge）
// 对照: src/utils/todo/types.ts TodoListSchema = z.array(TodoItemSchema())（无 list-level refinement —— 不变量是 prompt 软契约）
// 对照: src/tools/TodoWriteTool/prompt.ts:158 "Exactly ONE task must be in_progress"（唯一约束在 prompt 字面，runtime 不强制）
// 对照: src/utils/attachments.ts:254-257 TODO_REMINDER_CONFIG{TURNS_SINCE_WRITE:10,TURNS_BETWEEN_REMINDERS:10} + :3266-3317 getTodoReminderAttachments + :3212-3263 getTodoReminderTurnCounts
// 对照: src/utils/messages.ts:3663-3679 todo_reminder render（wrapMessagesInSystemReminder + isMeta）+ src/utils/sessionRestore.ts:77-93 extractTodosFromTranscript
// 核心论断：(1) 极简 tool（执行体只存取）+ 厚 prompt（行为协议全在 184 行 prompt）—— "状态由 prompt 决定，不在 code"
//          (2) 不变量是 prompt 软契约：call() 零 validation + schema 无 list-level refinement，"最多一个 in_progress" 只靠 model 自觉
//          (3) 三层 reinforcement：continuous（每次 tool_result 钉子）/ fixed-interval（缺席探测器，调用沉默时才 fire）/ event-triggered（verificationNudge）
//          (4) per-agent 隔离：todoKey = agentId ?? sessionId，sub-agent 自有 todo 不污染主 agent（与 v4 multi-agent context 二分同源）
//          (5) 无文件持久化：messages 数组即数据库，restore 靠倒扫 transcript 反序列化最后一次 TodoWrite（与 v5 compaction "messages 即真相" 同精神）
//          (6) 架构正交性第六次验证：obs/hook/permission/dispatch/compact 对 TodoWrite 字面 0 修改，obs 自动 cardinality-control tool_name=TodoWrite
// v12 教学简化：fixed-interval 阈值可 --todo-reminder-turns=N 覆盖（工业 10/10，demo 用小值让缺席可见）；verificationNudge 引导 spawn_swarm 作 verify 子任务（工业引导 VERIFICATION_AGENT_TYPE）；reminder piggyback 到上一条 user 消息（保持 role 交替，工业是独立 isMeta user message）；TodoV2 file-backed 路径不做
