...
# 5. compact 必须清 Session-Set（不调 model API / 确定性证明 §31 修复）
...
- `[CACHE CLEAR] memoryCache + LRU + session-set cleared by compact: N entries dropped (dedup gate reopened → all layers re-inject next round)` —— compact 后三者一起清 / 闸门重开（对齐工业 compact.ts:521-522）
...
- **compact-triggered clear 跟 [05-context-compactor](../05-context-compactor/) + [10-system-prompt-assembly](../10-system-prompt-assembly/) 同精神**：compact 后语义状态变 / cache 必失效 / **Session-Set 也一起清**（工业 compact.ts:521-522：readFileState 与 loadedNestedMemoryPaths 绑一起 clear / 否则 compact 后 nested CLAUDE.md 永久丢失）。注意「对 LRU 驱逐免疫(non-evicting)」≠「跨 compact 保留」—— 二者不矛盾
