...
# Demo E: compact 必须清 Session-Set（不调 model API / 确定性证明 §31 修复 / 见论断 4）
bun run ../agent-v13-claude-md-system.ts --demo=compact-reload
```

预期看到 4 类字面证据：
- `[MEMORY LOAD] cwd=... loaded=N layers=[User+Project*M+Local]` —— 上轨注入
- `[NESTED INJECT] ./subdir/CLAUDE.md (triggered by ./subdir/file.ts)` —— 下轨注入
- `[CACHE CLEAR] memoryCache + LRU + session-set cleared by compact: N entries dropped (dedup gate reopened → all layers re-inject next round)` —— compact 失效
...
让 model 顺序 read 5 个文件触发 fullCompact —— §31 `clearMemoryCache` 在 maybeCompact 末尾把 **memoryCache + LRU + Session-Set 三者一起清**。

为什么三者一起清（尤其 Session-Set）：compact 抹掉持有 nested CLAUDE.md 注入内容的历史 messages，内容随之蒸发。而 Session-Set 同时把守上轨 Project cascade（line 992）和下轨 nested（line 1047）的 dedup 闸门 —— 闸门若不重开，path 永远命中 `.has()` → **永不重注 → 指令永久丢失**。这正是工业 `compact.ts:521-522`（full）/ `:920-921`（partial）把 `readFileState` 与 `loadedNestedMemoryPaths` 绑在一起 clear 的原因（`postCompactCleanup.ts` 另清 module-level memo）。

用确定性 demo（`--demo=compact-reload` / 无需 API）验证 clear 前后闸门 `.has()` 翻转：

@include(./run-log-compact-reload-fix.txt, section="COMPACT-RELOAD DEMO")

观察：compact 前 `has(/proj/CLAUDE.md)=true`（闸门关）→ `clearMemoryCache()` 三者全清 → compact 后 `has(...)=false`（闸门重开）→ 下一轮上轨 `loadMemoryFiles` 缓存 miss 重算 + cascade **全层重注** / 下轨按 FileReadTool trigger 重注。**断言 PASS**。

跟 v10 `clearSystemPromptSections` 同精神：compact 后语义状态变 → 必须丢 cache 强制重算。**注意区分两种"不清"**：Session-Set 对 LRU 驱逐免疫（non-evicting，这是它相对 LRU 的存在意义，论断 3 lru-busy demo 证明的就是这层），但对 compact **不免疫** —— 前者是同会话 dedup 承诺，后者是会话语义重置。两件事不矛盾。
...
| 上轨 cascade 不查 Session-Set | 上轨 cascade 也查 Session-Set | 把 User/Project/Local 主层也接进统一 dedup 的化简；因 §31 compact 时三者一起清，主层下一轮照常重注，无副作用 |
